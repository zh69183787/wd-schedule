 ...
 * 
 */
public class ScheduleMain_copy {
	static Logger log = SimpleLogger.getLogger(ScheduleMain_copy.class);

	private static List<ScheduledFuture> futureList = new ArrayList<ScheduledFuture>();

	private ScheduleMain_copy() {
	...
	public static void addTask(TScheduleConfig t) {
		...
		if(!TimeUtil.isExec(t.getLastTime(), t.getTime(), t.getInterval())){
			ScheduledFuture f = scheduler.schedule(task,0,TimeUnit.SECONDS);
			futureList.add(f);
		}
	...
	public static String showStatus(){
		for(ScheduledFuture f : futureList){
